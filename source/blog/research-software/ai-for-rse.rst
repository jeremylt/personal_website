...
Note however that LLM code review appears to consume many tokens, according to Salim et al. [here](https://arxiv.org/abs/2601.14470), so this may be an expensive process if cost per token continues to rise.
...
Code review is one area that may be less risky for RSEs to use LLM based software development tools.
